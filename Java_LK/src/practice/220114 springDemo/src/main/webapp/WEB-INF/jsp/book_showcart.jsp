...
<table>
<tr><th>품목번호</th><th>제 목</th><th>가 격</th><th>출판사</th><th>수량</th></tr>
<c:forEach var="item" items="${list}">
   <tr><td>${item.no}</td>
      <td>${item.title}</td>
      <td>${item.price}</td>
      <td>${item.pub}</td>
      <td><input type="number" id="qty" name="qty" value="${item.qty}">
      <button onclick="edit(${item.no});">edit</button></td>
   </tr>
</c:forEach>
<fmt:formatNumber type="number" maxFractionDigits="3" 
               value="${total}" var="sum" />
   <tr id="footer"><td colspan="2">총 계<td colspan="3">${sum}원</td></tr>
</table>
<p>
